...
class RoomRepositoryImpl implements IRoomRepository {
  ...
  Future<List<Room>?> getAll() async {
    final entities = await _roomDataSource.getAllRooms();
    return RoomMapper.toDomainList(entities);
  }
  ...
  Future<List<Room>> getRoomsByType(String type) async {
    final entities = await _roomDataSource.getRoomsByType(type);
    return RoomMapper.toDomainList(entities);
  }

  @override
  Future<List<Room>> getAvailableRooms(DateTime dateTime) async {
    final entities = await _roomDataSource.getAvailableRooms(dateTime);
    return RoomMapper.toDomainList(entities);
  }
}